  ...
  return <section className="hellenic-calendar" aria-labelledby="calendar-title">
    ...
    <div className={`hellenic-calendar__body${annual ? ' is-annual' : ''}`}>
      {!annual && <div><div className="hellenic-calendar__month"><button type="button" aria-label="Mês anterior" disabled={year === 2026 && month === 0} onClick={() => moveMonth(-1)}><ChevronLeft /></button><h3>{MONTHS[month]} {year}</h3><button type="button" aria-label="Próximo mês" disabled={year === 2030 && month === 11} onClick={() => moveMonth(1)}><ChevronRight /></button></div>
        ...
        </div><p className="hellenic-calendar__legend"><span><i className="civil"/>Cívica</span><span><i className="orthodox"/>Ortodoxa</span></p>
      </div>}
    ...
    <details className="hellenic-calendar__sources"><summary>Sobre o calendário e leituras</summary><p>Festas fixas segundo o calendário grego de novo estilo, que coincide com o calendário civil neste período. Outras tradições ortodoxas podem observar festas fixas em dias diferentes. A Páscoa segue o ciclo pascal ortodoxo. Este calendário não inclui todos os santos do calendário diário nem feriados municipais ou regionais da Grécia.</p><p>As datas futuras são referências do calendário; não representam anúncios de suspensão de expediente. A observância de algumas datas, como a Segunda-feira do Espírito Santo, varia por setor.</p><ul><li><Link to="/cultura/calendario-grego-ortodoxo-pascoa-feriados">Guia do calendário grego e ortodoxo</Link></li><li><Link to="/cultura/feriados-gregos-datas-civicas-religiosas">Feriados gregos: datas cívicas e religiosas</Link></li><li><Link to="/cultura/pascoa-ortodoxa-2026-2030">Páscoa ortodoxa de 2026 a 2030</Link></li><li><Link to="/cultura/doze-grandes-festas-ortodoxas">As doze grandes festas ortodoxas</Link></li><li><Link to="/cultura/ano-liturgico-ortodoxo">O ano litúrgico ortodoxo</Link></li></ul></details>
  </section>
}
