...
const cards = editorialArticles.map(p=>`<a class="post-card" href="${articlePath(p.slug)}"><img ${imageVariants(p.featuredMedia, true)} src="${asset(p.featuredMedia.sourceUrl)}" alt="${esc(p.featuredMedia.alt)}" style="object-position:${esc(p.featuredMedia.position)}" loading="lazy" width="600" height="400"><div><span>Cultura e memória</span><h2>${esc(p.title)}</h2><p>${esc(p.excerpt)}</p><b>Ler artigo →</b></div></a>`).join('')
