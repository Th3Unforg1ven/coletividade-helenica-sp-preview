...
const imageVariants = (media, card = false) => media.illustration
  ? `srcset="${[640,960,1536].map(w => `${asset(media.sourceUrl.replace('.webp', w === 1536 ? '.webp' : `-${w}.webp`))} ${w}w`).join(', ')}" sizes="${card ? '(max-width: 560px) 100vw, (max-width: 1000px) 50vw, 33vw' : '(max-width: 1000px) 100vw, 50vw'}"`
  : ''
...
const nav = `<header class="editorial-header"><a href="${appPath('/')}"><img src="${asset('images/chsp-logo-256.png')}" alt="" width="52" height="52">Coletividade Helênica de São Paulo</a><nav aria-label="Navegação principal"><a href="${appPath('/coletividade')}">A Coletividade</a><a href="${appPath('/cultura')}">Cultura e memória</a><a href="${appPath('/cursos')}">Cursos</a><a href="${appPath('/agenda')}">Agenda</a><a href="${appPath('/contato')}">Contato</a></nav></header>`
...
for (const post of editorialArticles) {
  ...
  const body = `<main id="main-content" class="content-page editorial-static"><nav class="breadcrumbs" aria-label="Navegação estrutural"><a href="${appPath('/')}">Início</a><span>/ <a href="${appPath('/cultura')}">Cultura e memória</a></span></nav><header class="content-hero"><div><p class="content-kicker">Cultura e memória</p><h1>${esc(post.title)}</h1><p>${esc(post.excerpt)}</p></div><figure><img ${imageVariants(post.featuredMedia)} src="${asset(post.featuredMedia.sourceUrl)}" alt="${esc(post.featuredMedia.alt)}" style="object-position:${esc(post.featuredMedia.position)}" width="900" height="600">${post.featuredMedia.illustration?'<figcaption class="editorial-image-caption">Ilustração editorial</figcaption>':''}</figure></header><div class="content-layout"><article><p class="editorial-meta">Publicado em ${new Intl.DateTimeFormat('pt-BR',{timeZone:'America/Sao_Paulo'}).format(new Date(post.date))} · Leitura de ${minutes} min</p><nav class="editorial-toc" aria-label="Neste artigo"><strong>Neste artigo</strong><ol>${headings.map(h=>`<li><a href="#${h.id}">${h.title}</a></li>`).join('')}</ol></nav><div class="wp-content">${content}</div>${post.cta?`<a class="button" href="${href(post.cta.href)}">${esc(post.cta.label)}</a>`:''}</article></div></main>`
...
// Preserve old shared links without keeping a separate editorial section.
const culturePath = appPath('/cultura')
await fs.mkdir('dist/publicacoes',{recursive:true})
await fs.writeFile('dist/publicacoes/index.html',`<!doctype html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="robots" content="noindex"><meta http-equiv="refresh" content="0;url=${esc(culturePath)}"><title>Cultura e memória | CHSP</title></head><body><a href="${esc(culturePath)}">Ir para Cultura e memória</a></body></html>`)
await fs.writeFile('dist/sitemap.xml',`<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${urls.map(url=>`<url><loc>${esc(url)}</loc></url>`).join('')}</urlset>`)
console.log(`Generated ${urls.length} static articles, legacy redirect and sitemap.`)
